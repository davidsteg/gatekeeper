...
Fixes the Outlook tools' action and argument contract, which could not reach the CLI in two configurations. A toolkit that spells its allowed_microsoft_actions after its tool IDs (list_messages, get_message, list_folders, send_mail) loads clean, passes every Tier 1 check and then died on argparse's "invalid choice" for every call: execute_microsoft now translates those four names to the CLI words they mean (list/get/folders/send) when the argv is assembled, after the whitelist has had its say - so a read-only mailbox toolkit still refuses a send under either spelling, and an action name the table does not know is still passed through untouched. Second, an optional parameter the agent did not supply is now left off the argv instead of denying the call, so the CLI's own defaults apply (--folder inbox, --max 10) and a tool is usable exactly as its own MCP schema documents it; a positional with no value and an argument mapping naming an undeclared parameter are still denials. google_args and microsoft_args are one function now (validate._build_cli_args) rather than two copies. Third, a boolean parameter now has a mapping that works: `switch: --raw-query` is a valueless option declaration - true emits the fixed flag exactly once, false and "not supplied" emit nothing, and the boolean itself never reaches the argv, so there is no element to smuggle a second argument through. That is the shape argparse store_true options need (drive search --raw-query, mail list --unread, both --html), which reject the "--flag true" a valued mapping emits; `flag:` keeps meaning "takes a value" for every type, so no existing tool changes. A switch is validated at load rather than at call time, because unlike a valued entry it has nothing that could be missing: catalog._parse_cli_args refuses a switch that also sets flag/positional, one whose parameter is absent or not type boolean, and one whose flag is not a single option token (no whitespace, no "=", no placeholder). The google integration's drive.search starter tool gains the raw_query switch. Docs: architecture, toolkits.yaml and tools.yaml spell out the three names for one operation (tool ID, action ID, CLI argv) and the three argv shapes a mapping entry can take; the 0.46.3 note above named the Outlook tools with underscores, and is corrected to the dotted IDs that actually ship. New tests/test_cli_arg_contract.py covers the shared argv tail and the switch contract against both providers, censuses the shipped catalog (every boolean mapped as a switch, every switch behind a boolean), runs every shipped google and microsoft tool's minimal argv through the real CLI as a subprocess, and parses the switch argv with the two CLIs' own argparse grammars in-process - checking that --raw-query really arrives as raw_query is True and that --raw-query true really exits 2. No credential of any kind is involved, so nothing is sent.

## 0.46.3

Two changes. Google OAuth credentials now persist the scopes the token response granted - the refresh only asks for a subset of the grant, so tokens no longer die with RefreshError invalid_scope when the consent screen did not list every scope the fallback list assumed. Microsoft Outlook mail arrives as the second OAuth provider: a stdlib-only Graph v1.0 executor (mail list/get/folders/send via the existing google-executor shape), /ui/oauth/microsoft/authorize + callback with PKCE, offline_access kept on the stored grant, a shared _oauth_token_env materialization writing microsoft_token.json, and catalog tools outlook.list_messages / outlook.get_message / outlook.list_folders / outlook.send_mail.
...
- **Scopes come from Tier 1.** A `google` toolkit may declare `required_scopes:` (bare names like `gmail.send`, or full scope URLs); the consent screen asks for the union across every google toolkit, read at request time, so a redeploy that adds or narrows a toolkit changes the next sign-in with nothing else to keep in step. With none declared, the default set is asked for: `gmail.readonly`, `gmail.send`, `gmail.modify`, `calendar.events`, `calendar.events.readonly`, `drive`, `drive.file`, `drive.metadata.readonly`, `spreadsheets`, `contacts`. `required_scopes` on a non-`google` toolkit is refused at startup, like `run_as` on a non-`file` one. The gmail/calendar/drive starter integrations' toolkit YAML now carries the matching lists.
