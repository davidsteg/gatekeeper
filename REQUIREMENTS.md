...
- **FR-9.1a Resolved Command.** For the argv executors (`docker`, `local`, `ssh`) the record also carries `argv` — the command as executed, binary first, one element per template element (FR-5.3/5.4). The parameters do not determine it: between them and the process sit the tool's `argv` template, derived values (FR-5.5) and Tier 1. A definition whose *template* is wrong produces a perfectly healthy parameter set and a command that fails, so without this field the log cannot distinguish a broken definition from a bad argument — a distinction that has already cost a real misdiagnosis. `null` for the executors that build no argv (`http`, `truenas`, `google`, `file`) and for a rejection raised before the argv existed. This adds no exposure beyond FR-9.1: the argv is derived from the template plus the parameters already logged, credentials reach the executors through environment and headers rather than the command line (FR-10.7), and FR-10.6's masking covers the whole record.
