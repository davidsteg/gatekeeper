...
| `opencode` | A headless opencode coding-agent server | HTTP; one operation = one fixed multi-request workflow | **v1 active** |
...
- **FR-8.19** `run_as` is rejected on every other executor. `docker`, `local`, `http`, `truenas`, `ssh`, `google` and `opencode` are unaffected by the field's existence; a `run_as` on one of them aborts startup rather than being ignored, because a configuration that reads as "these operations run as somebody else" and does not is worse than one that refuses to start.
- **FR-8.20 The Agent Can Never Choose the User *at Call Time*.** There is no parameter, tool field, or header through which a call can pick who it runs as — the same rule FR-8.3i states for destinations, for the same reason. `run_as` is fixed on the toolkit before the call exists.
- **FR-8.20a** `run_as` MAY be changed by a human-reviewed toolkit proposal (`admin.toolkit_update`), alongside `executor`/`binaries`/`denied_args`. It was redeploy-only in 0.28.0; that ban did not survive contact with `admin.toolkit_propose`, which already carries a toolkit's *full* body including `path_roots` — an agent could propose a broad `file` toolkit and then update it, so the ban cost friction and bought no boundary. Two boundaries do the work instead, and neither is reachable from `/admin/mcp`: the container's own privilege to change user at all (FR-8.22 — without `CAP_SETUID`/`CAP_SETGID` a deployed `run_as` makes calls fail, not escalate), and `path_roots`/`protected_resources`, which stay redeploy-only. A proposal may therefore change *who* an operation runs as, never *where* it may reach.
...
opencode is a coding agent that runs as a headless server and exposes an HTTP API. Reaching it through `http` works endpoint by endpoint, but the useful unit of work is not an endpoint: asking a question is a session creation plus a message, running a task is a session, an async dispatch, and a poll loop until it finishes. Expressed as `http` tools that is three or four agent round trips per task, with the session id and every raw intermediate response spent on the agent's context. The executor exists so the boundary can be stated in terms of the workflow instead of the request — the same move `file` makes against `local`, one layer up.

- **FR-8.23** A tool on an `opencode` toolkit declares one fixed `opencode_operation` — `ask`, `run`, `fire`, `check`, `review_changes`, `abort`, `providers` or `health` — and nothing else. There is no method, path, query or body template: the request paths of each workflow live in the executor, in one table, and no tool definition or parameter composes them. The whitelist acts on **operation names**, exactly as FR-8.3c's does on JSON-RPC method names: an operation absent from a toolkit's list does not exist for it, and there is no separate permission to deny.
- **FR-8.24** The target rules of the `http` executor apply unchanged, because the transport is the same: scheme and host live exclusively in the toolkit's `base_url` (FR-8.5), the **resolved** IP is checked against `allowed_cidrs` immediately before connecting (FR-8.9) — per request, not once per workflow, since a workflow can span minutes and a single check at its start would leave exactly the rebinding window that requirement closes — redirects are reported and never followed (FR-8.8), the credential is injected as a header by the executor (FR-8.14), and responses are capped and marked external, untrusted data (FR-8.12/8.13).
- **FR-8.25** An operation's parameters are fixed names the executor reads, not a template it binds. It follows that a name outside the operation's set is rejected at load time rather than accepted and silently ignored — a `dir` typed for `directory` would otherwise be visible only as opencode working in the wrong repository.
- **FR-8.26** Exactly two parameter values leave the per-parameter allowlist behind, and both are checked before execution *and* again inside the executor (the doubling FR-4.6 already requires of argv). `session_id` is the only value that ever reaches a request path and is restricted to a character set that cannot introduce a path segment. `directory` — sent as opencode's own project-root header, which is what lets one toolkit serve many repositories without one toolkit per repository — must lie inside the toolkit's `path_roots`; a toolkit that declares none accepts no `directory` at all. That containment check is lexical rather than `realpath`-based, and its existence half runs only where the matching root is visible in gatekeeper's own filesystem: the path names a directory in the opencode container's namespace, so resolving symlinks would resolve them in the wrong one.
- **FR-8.27 A Timed-Out Workflow Is `unknown`, Not `failed`.** `run` dispatches the prompt and then polls. When the call's budget runs out, the prompt has reached opencode and the session keeps working on the other side — the exact case FR-6.9 reserves `unknown` for. The response says so and carries the session id, so the follow-up is `check`, never a retry that would start the same work a second time.
