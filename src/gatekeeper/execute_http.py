...
async def run(
    *,
    ...
    this module is one of exactly four places in the codebase allowed to
    call `CredentialStore._resolve` (the others are `execute_ssh.py`,
    `execute_truenas.py`, and `service.py`'s `_docker_tls_env` -- the
    docker executor has no per-call request object to thread a resolved
    credential through, so that one materialization has to live in
    `service.py` itself) -- keeping the decrypt call here for http, not
    duplicated elsewhere, is what makes that invariant grep-able instead
